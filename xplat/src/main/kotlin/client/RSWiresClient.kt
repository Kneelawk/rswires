...
object RSWiresClient {
    ...
    private val redAlloyOffModel = UnbakedWireModel(factory, Identifier.of(MOD_ID, "block/red_alloy_wire/off"), 0.125f, 0.125f, 32.0f)
    private val redAlloyOnModel = UnbakedWireModel(factory, Identifier.of(MOD_ID, "block/red_alloy_wire/on"), 0.125f, 0.125f, 32.0f)

    private val insulatedWireOffModel = DyeColor.entries.associateWith { UnbakedWireModel(factory, Identifier.of(MOD_ID, "block/insulated_wire/${it.getName()}/off"), 0.25f, 0.1875f, 32.0f) }
    private val insulatedWireOnModel = DyeColor.entries.associateWith { UnbakedWireModel(factory, Identifier.of(MOD_ID, "block/insulated_wire/${it.getName()}/on"), 0.25f, 0.1875f, 32.0f) }

    private val colorBundledCableModel = DyeColor.entries.associateWith { UnbakedWireModel(factory, Identifier.of(MOD_ID, "block/bundled_cable/${it.getName()}"), 0.375f, 0.25f, 32.0f) }
    private val plainBundledCableModel = UnbakedWireModel(factory, Identifier.of(MOD_ID, "block/bundled_cable/none"), 0.375f, 0.25f, 32.0f)
    ...
    @JvmStatic
    fun getUnbakedModel(modelId: ModelIdentifier): UnbakedModel? {
